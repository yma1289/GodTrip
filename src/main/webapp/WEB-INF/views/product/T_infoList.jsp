...
<div class="col-sm-12">
	...
	<tbody>
	  <c:forEach var="T_info" items="${T_infoList}">
	    <c:if test="${T_info.FT_code == 'T'}">
		  <tr class="text-center">
			<td>
				<c:choose>
					<c:when test="${T_info.FT_code == 'F'}">비행기</c:when>
					<c:when test="${T_info.FT_code == 'T'}">기차</c:when>
					<c:otherwise>${T_info.FT_code}</c:otherwise>
				</c:choose>
			</td>
			<td>${T_info.trans_code}</td>
			<td>${T_info.trans_name}</td>			
			<td>
				<c:choose>
					<c:when test="${T_info.filename != '-'}">
						<img src="/storage/${T_info.filename}" width="100px">						
					</c:when>
					<c:otherwise>
						등록된 사진 없음! <br>
					</c:otherwise>
...
<script>
	function confirmDelete(trans_code){
		if(confirm("정말로 삭제하시겠습니까?")){
			location.href='/partner/T_infoDelete?trans_code='+trans_code;
		}//if end
	}//confirmDelete() end
</script>
