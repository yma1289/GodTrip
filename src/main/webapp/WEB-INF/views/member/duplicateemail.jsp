...
<script>
 function apply(email){
    opener.document.getElementById('memfrm').elements.email.value=email;
    window.close();
}
</script>  

	<h2 style="text-align: center;">이메일 중복 검사 결과</h2>
	<p style="text-align: center;">검사한 이메일: ${email}</p>
	<c:choose>
		<c:when test="${cnt == 0}">
			<p style="text-align: center; color: blue">사용 가능한 이메일입니다.</p>
           <p style="text-align: center;"> <a href="javascript:apply('${email}')">적용</a></p>
        </c:when>
		<c:otherwise>
